...
> **기준 PRD:** `docs/PATENT/PRD.md` v1.1
...
- [ ] `C:\DATA\workspace\patentAI` 디렉토리에 CASA 프로젝트 클론
  ```bash
  cp -r C:\DATA\workspace\casa\* C:\DATA\workspace\patentAI\
  ```
- [ ] `package.json` → `name` 필드를 `"patent-ai"`로 변경
- [ ] `CLAUDE.md` → PatentAI 용으로 내용 업데이트
- [ ] 불필요한 기존 CASA 페이지 제거
  - `src/app/[locale]/(public)/` — 로그인/회원가입 유지
  - `src/app/[locale]/dashboard/` — 내용 비우고 PatentAI 대시보드로 교체
  - `src/app/[locale]/admin/` — 일단 유지 (프롬프트 관리 등 재사용)
  - `src/features/` — 기존 CASA 기능 폴더 삭제 후 `patent/` 폴더 신규 생성

### P0-2. 환경변수 설정
- [ ] `.env.local` 생성 (CASA `.env.local` 복사 후 수정)
  ```env
  ...
  ```
- [ ] `.env.local`을 `.gitignore`에 추가 확인 (CASA에서 이미 설정됨)

### P0-3. 의존성 추가
- [ ] KIPRIS/USPTO 연동용 패키지 확인 (별도 SDK 없음 — fetch 직접 사용)
- [ ] `mammoth` 패키지 추가 (DOCX → 텍스트 추출)
  ```bash
  npm install mammoth
  npm install @types/mammoth --save-dev
  ```
- [ ] 기존 의존성 그대로 유지 (CASA 의존성 패키지 전부 재사용)
...
### P1-2. 핵심 테이블 생성
- [ ] `bi_patent_projects` 테이블:
  ```sql
  CREATE TABLE bi_patent_projects (
  ...
  CREATE POLICY "owner_all" ON bi_patent_projects
    USING (user_id = auth.uid());
  ```

- [ ] `bi_patent_inputs` 테이블:
  ```sql
  CREATE TABLE bi_patent_inputs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  CREATE POLICY "owner_all" ON bi_patent_inputs
    USING (EXISTS (
...
- [ ] `bi_patent_components` 테이블:
  ```sql
  CREATE TABLE bi_patent_components (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID NOT NULL REFERENCES bi_patent_projects(id) ON DELETE CASCADE,
  ...
  CREATE POLICY "owner_all" ON bi_patent_components
    USING (EXISTS (
      SELECT 1 FROM bi_patent_projects p
      WHERE p.id = project_id AND p.user_id = auth.uid()
    ));
...
- [ ] `bi_patent_prior_art` 테이블:
  ```sql
  CREATE TABLE bi_patent_prior_art (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID NOT NULL REFERENCES bi_patent_projects(id) ON DELETE CASCADE,
    ...
    conflicting_component_ids JSONB DEFAULT '[]',  -- bi_patent_components.id 배열
    avoidance_suggestion TEXT,
    searched_at TIMESTAMPTZ DEFAULT now()
  );

  ALTER TABLE bi_patent_prior_art ENABLE ROW LEVEL SECURITY;
  CREATE POLICY "owner_all" ON bi_patent_prior_art
    USING (EXISTS (
      SELECT 1 FROM bi_patent_projects p
      WHERE p.id = project_id AND p.user_id = auth.uid()
    ));
...
- [ ] `bi_patent_claims` 테이블:
  ```sql
  CREATE TABLE bi_patent_claims (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID NOT NULL REFERENCES bi_patent_projects(id) ON DELETE CASCADE,
    claim_number INT NOT NULL,
    claim_type patent_claim_type NOT NULL DEFAULT 'independent',
    parent_claim_id UUID REFERENCES bi_patent_claims(id),
  ...
  CREATE POLICY "owner_all" ON bi_patent_claims
    USING (EXISTS (
      SELECT 1 FROM bi_patent_projects p
      WHERE p.id = project_id AND p.user_id = auth.uid()
    ));
...
- [ ] `bi_patent_sections` 테이블:
  ```sql
  CREATE TABLE bi_patent_sections (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID NOT NULL REFERENCES bi_patent_projects(id) ON DELETE CASCADE,
  ...
  CREATE POLICY "owner_all" ON bi_patent_sections
    USING (EXISTS (
      SELECT 1 FROM bi_patent_projects p
      WHERE p.id = project_id AND p.user_id = auth.uid()
    ));
...
- [ ] `bi_patent_drawings` 테이블:
  ```sql
  CREATE TABLE bi_patent_drawings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID NOT NULL REFERENCES bi_patent_projects(id) ON DELETE CASCADE,
  ...
  CREATE POLICY "owner_all" ON bi_patent_drawings
    USING (EXISTS (
      SELECT 1 FROM bi_patent_projects p
      WHERE p.id = project_id AND p.user_id = auth.uid()
    ));
...
- [ ] `bi_patent_gates` 테이블:
  ...
  CREATE POLICY "owner_all" ON bi_patent_gates
    USING (EXISTS (
      SELECT 1 FROM bi_patent_projects p
      WHERE p.id = project_id AND p.user_id = auth.uid()
    ));
...
- [ ] Supabase Dashboard > Storage > New Bucket
  ...
  - Public: **OFF** (비공개)
  - 파일 크기 제한: 50MB
  - 허용 MIME: `application/pdf`, `application/vnd.openxmlformats-officedocument.wordprocessingml.document`, `text/markdown`, `text/plain`
- [ ] 버킷명: `patent-drawings`
  - Public: **OFF**
  - 허용 MIME: `image/png`, `image/jpeg`, `image/webp`

### P1-4. 프롬프트 등록 (`bi_prompts` 테이블)
> CASA 프롬프트 엔진 완전 재사용. DB에 특허용 프롬프트 키만 추가.

- [ ] SQL로 프롬프트 삽입:
  ```sql
  -- STEP 1: 기술 분석
  INSERT INTO bi_prompts (key, name, system_prompt, user_prompt_template, model, credit_cost)
  VALUES (
...
### P2-1. 재사용 파일 목록 확인
- [ ] `src/lib/ai/claude.ts` — Claude API 래퍼 (callClaude, streamClaude, createSSEResponse)
- [ ] `src/lib/ai/gemini.ts` — Gemini API 래퍼 (이미 구현됨)
- [ ] `src/lib/ai/openai.ts` — OCR 폴백용
- [ ] `src/lib/ai/index.ts` — 멀티 프로바이더 통합
- [ ] `src/lib/prompts/prompt-engine.ts` — DB 기반 프롬프트 (Redis 캐싱)
- [ ] `src/lib/prompts/version-manager.ts` — 프롬프트 버전 관리
- [ ] `src/lib/credits.ts` — 크레딧 차감/충전
- [ ] `src/lib/auth/guards.ts` — requireAuth, requireProjectOwner
- [ ] `src/lib/utils/api-response.ts` — successResponse, errorResponse
- [ ] `src/lib/utils/document-export.ts` — exportToDocx, exportToPdf
- [ ] `src/lib/utils/pdf-extract.ts` — extractTextFromPdf, renderPdfToImages
- [ ] `src/lib/redis.ts` — Upstash Redis 클라이언트
- [ ] `src/lib/supabase/client.ts` / `server.ts` / `service.ts`
- [ ] `src/hooks/useSSE.ts` — SSE 스트리밍 훅
- [ ] `src/hooks/usePaginatedFetch.ts` — 페이지네이션 훅
- [ ] `src/components/ui/*.tsx` — shadcn/ui 전체

### P2-2. DOCX 텍스트 추출 추가 (신규)
- [ ] `src/lib/utils/docx-extract.ts` 신규 생성:
  ```typescript
  // mammoth 라이브러리 활용
...
### P3-1. API: 프로젝트 생성
- [ ] `src/app/api/patents/route.ts` 신규
  - `POST`: 새 특허 프로젝트 생성 (`bi_patent_projects` INSERT)
  - `GET`: 내 프로젝트 목록 조회 (페이지네이션)
  - **재사용**: `requireAuth`, `successResponse`, `errorResponse`

### P3-2. API: 파일 업로드
- [ ] `src/app/api/patents/[id]/upload/route.ts` 신규
...
### P3-6. GATE 1 UI
- [ ] `src/features/patent/gates/Gate1.tsx` 신규
  - 분석 결과 확인 화면
  - IPC 코드 드롭다운 선택/수정
  - 핵심 발명 포인트 인라인 편집
  - 버튼: ✏️ 수정 / 🔄 재분석 / ✅ 확인하고 다음으로
- [ ] `src/app/api/patents/[id]/gates/1/route.ts` 신규
  - `POST`: GATE 1 승인 처리
  - `bi_patent_gates` INSERT (gate_number=1, status='approved')
  - `bi_patent_projects` status → `'step1_done'`
...
- [ ] `src/app/api/patents/[id]/components/generate/route.ts` 신규
  - 기술 분석 결과 기반으로 구성요소 트리 자동 생성
  - 프롬프트 키 `patent_component_structuring` 사용
  - `bi_patent_components` 일괄 INSERT
  - **재사용**: `streamClaude`, `createSSEResponse`, `preparePrompt`

### P4-2. API: 구성요소 CRUD
- [ ] `src/app/api/patents/[id]/components/route.ts` 신규
  - `GET`: 전체 구성요소 트리 조회 (계층 구조)
  - `POST`: 구성요소 추가
- [ ] `src/app/api/patents/[id]/components/[compId]/route.ts` 신규
  - `PATCH`: 구성요소 수정 (이름, 설명, 참조번호, 순서)
  - `DELETE`: 구성요소 삭제 (자식 포함)

### P4-3. UI: 구성요소 트리 편집기
- [ ] `src/features/patent/step2/ComponentTree.tsx` 신규
  - 계층적 트리 렌더링 (재귀 컴포넌트)
  - 참조번호 배지 표시 (100, 110, 111 등)
  - 선행기술 충돌 시 ⚠️ 아이콘 표시 (GATE 2 후 업데이트)
  - 인라인 편집 (이름, 설명 클릭 시 input 전환)
  - 추가/삭제 버튼
- [ ] `src/features/patent/step2/DrawingPlan.tsx` 신규
  - AI 추천 도면 목록 표시
  - 도면 유형 선택/추가/제거
...
### P5-1. KIPRIS API 연동
- [ ] `src/lib/prior-art/kipris.ts` 신규
  ```typescript
...
### P5-3. 유사도 분석 서비스
- [ ] `src/lib/prior-art/similarity.ts` 신규
  - 구성요소 키워드 추출
  - KIPRIS + USPTO 동시 검색 (`Promise.all`)
  - Claude API로 유사도 점수 산출 (임베딩 없이 텍스트 비교 방식)
  - `bi_patent_prior_art` INSERT

### P5-4. API: 선행기술 조사
- [ ] `src/app/api/patents/[id]/prior-art/search/route.ts` 신규
  - `POST`: 선행기술 조사 실행
  - 구성요소 + IPC 코드 기반 검색
  - 결과를 `bi_patent_prior_art`에 저장
  - 충돌 컴포넌트 `bi_patent_components.has_prior_art_conflict` 업데이트

### P5-5. API: 회피 전략 생성 (SSE)
- [ ] `src/app/api/patents/[id]/prior-art/avoidance/route.ts` 신규
  - 프롬프트 키 `patent_prior_art_avoidance` 사용
  - 충돌 위험 컴포넌트별 회피 방안 생성
  - `bi_patent_projects.overall_prior_art_risk` 업데이트
  - **재사용**: `streamClaude`, `createSSEResponse`, `preparePrompt`

### P5-6. UI: GATE 2 — 2패널 화면
- [ ] `src/features/patent/gates/Gate2.tsx` 신규
  - **좌측 패널**: 구성요소 트리 (Step2 ComponentTree 재사용)
  - **우측 패널**: 선행기술 조사 결과
    - 유사 특허 카드 목록 (patent_number, title, 유사도 배지)
    - 위험도별 색상: 빨강(high) / 주황(medium) / 초록(low)
    - 충돌 위험 없을 시 패널 자동 접힘 (`<Accordion>` 재사용)
    - 충돌 발견 시 자동 펼침
    - 💡 회피 전략 제안 표시
  - 버튼: ✏️ 구성요소 수정 / 🔄 재검색 / ✅ 확정하고 청구범위 작성
- [ ] `src/app/api/patents/[id]/gates/2/route.ts` 신규
  - GATE 2 승인 처리
  - `bi_patent_projects` status → `'step2_done'`
...
- [ ] `src/app/api/patents/[id]/claims/generate/route.ts` 신규
  - `POST body`: `{ claim_type: 'apparatus' | 'method' | 'system' }`
  - 구성요소 트리 + 회피 전략 컨텍스트 조합
  - 프롬프트 키 `patent_claims_generation` 사용
  - 청구항 JSON 스트리밍 후 `bi_patent_claims` 일괄 INSERT
  - **재사용**: `streamClaude`, `createSSEResponse`, `preparePrompt`

### P6-2. API: 청구항 CRUD
- [ ] `src/app/api/patents/[id]/claims/route.ts` 신규
  - `GET`: 전체 청구항 목록 (claim_number 순)
  - `POST`: 청구항 추가
- [ ] `src/app/api/patents/[id]/claims/[claimId]/route.ts` 신규
  - `PATCH`: 청구항 내용 수정 / 번호 변경
  - `DELETE`: 청구항 삭제 (종속 청구항 cascade 처리)
- [ ] `src/app/api/patents/[id]/claims/reorder/route.ts` 신규
  - `POST`: 청구항 순서 일괄 변경

### P6-3. API: 청구항 강도 분석
- [ ] `src/app/api/patents/[id]/claims/strength/route.ts` 신규
  - 전체 청구항 검사:
    - 금지 표현 탐지 (최선, 최고, 약, 대략, 다수의 등)
    - 참조번호 누락 탐지
    - 독립항 말미 "~특징으로 하는 ~." 패턴 확인
  - `bi_patent_claims.strength_score`, `strength_issues` UPDATE

### P6-4. UI: 청구항 편집기
- [ ] `src/features/patent/step3/ClaimsEditor.tsx` 신규
  - 청구항 목록 (독립항 굵게 표시, 종속항 들여쓰기)
  - 강도 점수 배지 (●●●●○ 형태)
  - 인라인 텍스트 편집 (`<textarea>` 자동 리사이즈)
  - 문제 표현 빨간 밑줄 하이라이팅
  - 청구항 트리 시각화 (종속 관계 화살표)
  - 버튼: 청구항 추가 / 삭제 / 재생성
  - **재사용**: `badge.tsx`, `accordion.tsx`

### P6-5. GATE 3 UI
- [ ] `src/features/patent/gates/Gate3.tsx` 신규
  - 청구항 전체 미리보기 (최종 확인)
  - 강도 분석 요약 (경고 항목 수)
  - 버튼: ✏️ 청구항 편집 / 🔄 재생성 / ✅ 청구범위 확정 및 본문 작성
- [ ] `src/app/api/patents/[id]/gates/3/route.ts` 신규
  - `bi_patent_claims.is_confirmed = true` (전체)
  - `bi_patent_projects` status → `'step3_done'`
...
- [ ] `src/app/api/patents/[id]/sections/generate/route.ts` 신규
  ...
  - 섹션 유형별 프롬프트 키 매핑:
    ...
    ```
  - SSE 스트리밍 중 `bi_patent_sections` UPSERT
...
- [ ] `src/app/api/patents/[id]/gates/4/route.ts` 신규
  ...
  - `bi_patent_projects` status → `'step4_done'`
...
- [ ] `src/app/api/patents/[id]/drawings/generate/route.ts` 신규
  ...
  - `bi_patent_drawings` INSERT (prompt_used, image_url 포함)
...
- [ ] `src/app/api/patents/[id]/gates/5/route.ts` 신규
  - 도면 최소 1개 이상 있는지 검증
  - `bi_patent_drawings.is_confirmed = true` (전체)
  - `bi_patent_projects` status → `'step5_done'`
...
- [ ] `src/app/api/patents/[id]/export/docx/route.ts` 신규
  - `bi_patent_sections` 전체 조회 → 섹션 순서대로 조합
...
- [ ] `src/features/patent/step6/ExportView.tsx` 신규
  ...
  - 다운로드 버튼
  - `bi_patent_projects` status → `'completed'`
